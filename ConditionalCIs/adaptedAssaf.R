...
# this functions computes P(|X|>c) as a function of theta. 
# It is called by functions below
...
Shortest.AR <- function(theta,cutoff,alpha){
	Q <- function(theta) 1 - pnorm(cutoff + theta) + 1 - pnorm(cutoff - theta)	
	...
	if (theta<0) {
		tmp <- Shortest.AR(-theta,cutoff,alpha)
		A <- -rev(tmp$A)
		l <- tmp$l
	}
       	else{
...
## Testing:
Shortest.AR(theta=1, cutoff=1, alpha=0.05)




# inputs are: x (value of the observation, |x|>cutoff), sigsq (a known value for the variance of X), cutoff (positive threshold) and alpha (level of the test)
Shortest.CI <- function(x, sigsq, cutoff, alpha) {
	stopifnot(abs(x) > cutoff)

	if (x<0) {
		tmp <- Shortest.CI(-x,sigsq,cutoff,alpha)
		lower <- -tmp$upper
		upper <- -tmp$lower
	} else {
...
## Testing:
Shortest.CI(x=1.9, sigsq=1, cutoff=1, alpha=0.01) 
...
MP.AR <- function(theta,r,cutoff,alpha){
	Q <- function(theta) 1 - pnorm(cutoff + theta) + 1 - pnorm(cutoff - theta)
	#compute useful quantities
	f <- function(theta) ( pnorm(cutoff + theta) - pnorm(cutoff - theta) ) - (1 - alpha) * Q(theta)
	theta1 <- uniroot(f,c(0,cutoff + qnorm(1 - alpha))) $ root
	f <- function(theta) pnorm( cutoff + r * Shortest.AR(theta,cutoff,alpha)$l - theta ) - pnorm( cutoff - theta ) - (1 - alpha) *  Q(theta)
	thetatilde1 <- uniroot(f,c(0,theta1)) $ root
	thetatilde2 <- uniroot(f,c(theta1, r * 2 * qnorm(1 - alpha / 2)))$root

	if (theta<0) {
		tmp <- MP.AR(-theta,r,cutoff,alpha)
		A <- -rev(tmp)
	} else{
		...
		if (0 < theta && theta < thetatilde1) {
			l <- Shortest.AR(theta,cutoff,alpha)$l
			f <- function(x) 1 - pnorm(theta - x) + 1 - pnorm(cutoff + r * l - (-cutoff - x) - theta) - alpha *  Q(theta)
			atilde1 <- uniroot(f,c(theta - qnorm(1 - alpha/2 *  Q(theta)), -cutoff))$root
			ll <- atilde1
			ul <- -cutoff
			lr <- cutoff
			ur <- cutoff + r * l - (-cutoff - atilde1)
		}
...
MP.CI <- function(x,sigsq,r,cutoff,alpha) {

	if (x<0) {
		tmp <- MP.CI(-x,sigsq,r,cutoff,alpha)
		lower <- -tmp$upper
		upper <- -tmp$lower
	} else {
		...
		lzero <- Shortest.AR(0,cutoff,alpha)$l
		f <- function(x) 1 - pnorm(x) + 1 - pnorm(2 * cutoff + r * lzero - x) - 2 * alpha * (1 - pnorm(cutoff))
		xtilde1 <- uniroot(f,c(cutoff,zalphahalf))$root
		xtilde2 <- uniroot(f,c(zalphahalf,cutoff + r * 2 * qnorm(1 - alpha / 2)))$root
		ltilde1 <- Shortest.AR(thetatilde1,cutoff,alpha)$l
		...
		f <- function(theta) 1 - pnorm(x - theta) + 1 - pnorm(theta - x + 2 * cutoff + r * Shortest.AR(theta,cutoff,alpha)$l) - alpha *  Q(theta)
		...
		f <- function(theta) pnorm(x - theta) - pnorm(x - r * Shortest.AR(theta,cutoff,alpha)$l - theta) - (1 - alpha) *  Q(theta)
		m <- optimize(f, c(thetatilde1,x), maximum=T)$maximum
		lower <- uniroot(f, c(thetatilde1,m))$root
		}
		#obtain upper end of CI
		f <- function(theta) pnorm(x + r * Shortest.AR(theta,cutoff,alpha)$l - theta) - pnorm(x - theta) - (1 - alpha) *  Q(theta)
...
# inputs are: theta (value of location parameter), lambad (positive penalty term for length of the AR), cutoff (positive threshold) and alpha (level of the test)
QC.AR <- function(theta,lambda,cutoff,alpha){
Q <- function(theta) 1 - pnorm(cutoff + theta) + 1 - pnorm(cutoff - theta)
#compute useful quantities
f <- function(theta) ( pnorm(cutoff + theta) - pnorm(cutoff - theta) ) - (1 - alpha) * Q(theta)
theta1 <- uniroot(f,c(0, cutoff + qnorm(1 - alpha))) $ root
...
if (theta<0) {
	tmp <- QC.AR(-theta,lambda,cutoff,alpha)
	A <- -rev(tmp)
} else {
	...
	dbar <- -cutoff - Shortest.AR(theta,cutoff,alpha)$A[1]
	...
	if (theta > theta1) {
	A <- Shortest.AR(theta,cutoff,alpha)$A
	ll <- A[1]
	ul <- A[2]
	lr <- A[3]
	ur <- A[4]
...
# inputs are: x (value of observation, |x|>cutoff), sigsq (a known value for the variance of X), lambada (positive pentalty term for length of the AR), cutoff (positive threshold) and alpha (level of the test)
QC.CI <- function(x,sigsq,lambda,cutoff,alpha){

	if (x<0) {
		tmp <- QC.CI(-x,sigsq,lambda,cutoff,alpha)
		lower <- -tmp$upper
		upper <- -tmp$lower
	} else {
		...
		dmax <- function(theta) -cutoff - Shortest.AR(theta,cutoff,alpha)$A[1]
		#compute useful quantities
		zalphahalf <- qnorm( 1 - .5 * alpha * Q(0) )
		dminzero <- dmin(0)
		dmaxzero <- dmax(0)
		...
		if (x >= xprime3 ) {
		lower <- Shortest.CI(x,cutoff,alpha)$lower
		}
...
QC.maxlength <- function(lambda,cutoff,alpha){
Q <- function(theta) 1 - pnorm(cutoff + theta) + 1 - pnorm(cutoff - theta)
dmin <- function(theta) max( -cutoff - theta + qnorm(1 - alpha * Q(theta)), 0 )
dmax <- function(theta) -cutoff - Shortest.AR(theta,cutoff,alpha)$A[1]
f <- function(d) 1 + lambda * ( 1 - dnorm(cutoff + d) / dnorm(qnorm(2 - pnorm(cutoff + d) - alpha * Q(0))) )
...
xprime2 <- qnorm( 2 - pnorm(xprime1) - alpha * Q(0) )
a <- QC.CI(xprime2,lambda,cutoff,alpha) $ lower
b <- QC.CI(xprime2,lambda,cutoff,alpha) $ upper
l <- b - a
return(l)
}
...
# lambda <- 2
# QC.AR(.5,lambda=2,cutoff,alpha) # The AR is a disjoint union of an interval lying to the left of -cutoff and an interval lying to the right of c. output lists lower left, upper left, lower right, upper right endpoints, respectively.
# QC.CI(1.7, sigsq=4,lambda=2,cutoff,alpha)
