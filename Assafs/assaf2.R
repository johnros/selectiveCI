...
# Note that the function which computes the CI can be run independently of the function which computes the AR, that is, the CI function does not call the AR function.
...
# inputs are: x (value of the observation, |x|>c), c (positive threshold) and alpha (level of the test)
...
# r (the maximum of the ratio between the MP interval and the usual two-sided interavl), 
# c (positive threshold) and alpha (level of the test)
...
# inputs are: x (value of observation, |x|>c), r (the maximum of the ration between the MP interval and the usual two-sided interavl), c (positive threshold) and alpha (level of the test)
...
# inputs are: theta (value of location parameter), 
# lambad (pentalty term for length of the AR), 
# c (positive threshold) and 
...
# inputs are: x (value of observation, |x|>c), 
# lambad (pentalty term for length of the AR), 
# c (positive threshold) and 
# alpha (level of the test)
...
# # QC method:
# lambda <- 2
# QC.AR(.5,lambda,c,alpha) # The AR is a disjoint union of an interval lying to the left of -c and an interval lying to the right of c. output lists lower left, upper left, lower right, upper right endpoints, respectively.
# QC.CI(1.25,lambda,c,alpha) # Output lists the lower and upper endpoint of the CI
